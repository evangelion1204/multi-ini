...
describe('Filters', function () {
    ...
    describe('constants', function () {
        ...
        it('string should have replaced constants', function () {
            ...
            expect(MultiIni.filters.constants('"Test " CONSTANT " Test"', options)).to.equal(
                '"Test replaced Test"',
            );
    ...
    describe('integer', function () {
        it('is available', function () {
            expect(MultiIni.filters.integer).to.be.defined;
        });

        it('should convert any number', function () {
            expect(MultiIni.filters.integer('0')).to.equal(0);
            expect(MultiIni.filters.integer('1')).to.equal(1);
            expect(MultiIni.filters.integer('-9')).to.equal(-9);
            expect(MultiIni.filters.integer('01')).to.equal(1);
        });

        it('should ignore numbers with leading 0 or non numbers', function () {
            expect(MultiIni.filters.integer('hello')).to.equal('hello');
            expect(MultiIni.filters.integer('false')).to.equal('false');
            expect(MultiIni.filters.integer('01', { keep_zero_prefix: true })).to.equal('01');
        });
    });
});
