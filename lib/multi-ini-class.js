...
MultiIni = (function() {
  ...
  MultiIni.prototype.getKeyValue = function(line, options) {
    var result;
    ...
    if (result) {
      ...
      return [result[1], result[2], result[3].length > 0 ? this.STATUS_INVALID : this.STATUS_OK];
    }
  ...
  MultiIni.prototype.getMultiKeyValue = function(line, options) {
    var result;
  ...
  MultiIni.prototype.getMultiLineEndValue = function(line, options) {
    var result;
    ...
    if (result) {
      return [result[1], result[2].length > 0 ? this.STATUS_INVALID : this.STATUS_OK];
    }
  };
  ...
  MultiIni.prototype.read = function(filename, options) {
    ...
    for (_i = 0, _len = lines.length; _i < _len; _i++) {
      ...
      if (this.isComment(line)) {
        ...
        _ref = this.getKeyValue(line, options), key = _ref[0], value = _ref[1], status = _ref[2];
        if (status === this.STATUS_INVALID && !options.oninvalid(line)) {
          return;
        }
        if (status === this.STATUS_INVALID && options.ignore_invalid) {
          continue;
        }
        keys = key.split('.');
        this.assignValue(current, keys, value);
      } else if (this.isMultiLine(line)) {
        _ref1 = this.getMultiKeyValue(line, options), key = _ref1[0], value = _ref1[1];
        keys = key.split('.');
        multiLineKeys = keys;
        multiLineValue = value;
      } else if (multiLineKeys) {
        if (this.isMultiLineEnd(line)) {
          _ref2 = this.getMultiLineEndValue(line, options), value = _ref2[0], status = _ref2[1];
          if (status === this.STATUS_INVALID && !options.oninvalid(line)) {
            return;
          }
          if (status === this.STATUS_INVALID && options.ignore_invalid) {
